...
Scalar Image::Max()
{
	...
	{
		M = std::max(std::max(M, this->data[i].x), std::max(this->data[i].y, this->data[i].z));
	}
	return M;
}

void Image::Multiply(Scalar scalar)
{
	for(int i = 0; i < width*height; i++)
	{
...
void Image::Transform(Colour (*mapFunction)(const Colour& c))
{
	for(int i = 0; i < width*height; i++)
		data[i] = mapFunction(data[i]);
}

Colour power_law(const Colour& c)
{
	return Colour(std::sqrt(c.x), std::sqrt(c.y), std::sqrt(c.z));
}

void Image::EyeResponseTransform1()
{
	this->Transform(power_law);
}

bool Image::SaveAsRaw(const char* filename)
{
	...
	for(int j = 0; j < height; j++)
	{
		...
		{
			file << data[i+j*width].x << " " << data[i+j*width].y << " " << data[i+j*width].z << std::endl;
		}
		file << std::endl;
	}
	return true;
...
Scalar clamp(Scalar x) { if(x < 0) return 0; if(x > 1) return 1; return x; }

bool Image::SaveAsBmp(const char* filename)
{
	bmpfile_t* bmpImage = bmp_create(width, height, 24);
	...
	for(int j = 0; j < height; j++)
	{
		for(int i = 0; i < width; i++)
		{
			...
			pixel.red = int(clamp(data[idx].x)*255);
			pixel.green = int(clamp(data[idx].y)*255);
			pixel.blue = int(clamp(data[idx].z)*255);
			pixel.alpha = 255;
			bmp_set_pixel(bmpImage, i, j, pixel);
		}
	}

	bool ret = bmp_save(bmpImage, filename);
	bmp_destroy(bmpImage);
	return ret;
}
